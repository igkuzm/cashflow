		...
		const char * predicate)
{
	...
	/*salary*/           ", salary"
	/*dividents*/        ", (SELECT 0 + SUM(income) FROM cashflow_actives WHERE type = %d AND cashflow_uuid = '%s')"
    /*rent*/             ", (SELECT 0 + SUM(income) FROM cashflow_actives WHERE type = %d AND cashflow_uuid = '%s')"
    /*business*/         ", (SELECT 0 + SUM(income) FROM cashflow_actives WHERE type = %d AND cashflow_uuid = '%s')"
    /*taxes*/            ", taxes"
    /*mortgage*/         ", (SELECT 0 + SUM(expenses) FROM cashflow_passives WHERE type = %d AND cashflow_uuid = '%s')"
    /*education_credit*/ ", (SELECT 0 + SUM(expenses) FROM cashflow_passives WHERE type = %d AND cashflow_uuid = '%s')"
    /*car_credit*/       ", (SELECT 0 + SUM(expenses) FROM cashflow_passives WHERE type = %d AND cashflow_uuid = '%s')"
    /*creditcard*/       ", (SELECT 0 + SUM(expenses) FROM cashflow_passives WHERE type = %d AND cashflow_uuid = '%s')" 
    /*some_credits*/     ", (SELECT 0 + SUM(expenses) FROM cashflow_passives WHERE type = %d AND cashflow_uuid = '%s')"
    /*other_expenses*/   ", other_expenses"
    /*child_cost*/       ", child_cost" 
